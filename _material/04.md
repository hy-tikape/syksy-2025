...
SELECT
  name,
  score,
  (SELECT COUNT(*) FROM Results WHERE score > R.score)
FROM
  Results R;
```
...
```
name        score       (SELECT COUNT(*) FROM Results WHERE score > R.score)
----------  ----------  -----------------------------------------------------
Uolevi      120         0                                                    
Maija       80          3                                                    
Liisa       120         0                                                    
...
Koska taulu `Results` esiintyy kahdessa roolissa alikyselyssä, pääkyselyn taululle on annettu nimi `T`. Tämän ansiosta alikyselyssä on selvää, että halutaan laskea rivejä, joiden tulos on parempi kuin pääkyselyssä käsiteltävän rivin tulos.
...
```sql
SELECT MAX(c) FROM (
  SELECT COUNT(*) c FROM Movies GROUP BY release_year
);
```
...
```sql
SELECT COUNT(*) c FROM Movies GROUP BY release_year ORDER BY c DESC LIMIT 1;
```
...
```sql
SELECT
  (SELECT COUNT(*) FROM Results WHERE score > T.score) + 1 place,
  name, score
FROM
  Results T
ORDER BY
...
```sql
SELECT
  (SELECT COUNT(*) FROM Results WHERE score > T.score OR
    (score = T.score AND name < T.name)) + 1 place,
  name, score
FROM
  Results T
ORDER BY
