...
Rakennamme tietokannan niin, että siinä on kolme taulua: `Products`, `Customers` ja `Purchases`. Liitostaulu `Purchases` ilmaisee, mitä tuotteita on kunkin asiakkaan ostoskorissa. Sen jokainen rivi esittää yhden parin muotoa "asiakkaan _id_ korissa on tuote _id_".
...
SELECT
  C.name, Pr.name
FROM
  Customers C, Products Pr, Purchases Pu
WHERE
  C.id = Pu.customer_id AND Pr.id = Pu.product_id;
```
...
```sql
SELECT * FROM Customers C, Products Pr, Purchases Pu;
```
...
FROM
  Customers C, Products Pr, Purchases Pu
WHERE
  C.id = Pu.customer_id AND Pr.id = Pu.product_id AND C.name = 'Maija';
```
...
FROM
  Customers C, Products Pr, Purchases Pu
WHERE
  C.id = Pu.customer_id AND Pr.id = Pu.product_id AND Pr.name = 'selleri';
```
...
```sql
SELECT
  C.name, COUNT(Pr.id), SUM(T.price)
FROM
  Customers C, Products Pr, Purchases Pu
WHERE
  C.id = Pu.customer_id AND Pr.id = Pu.product_id
...
Kun kyselyyn lisätään ryhmittely `GROUP BY C.id`, rivit jakautuvat kahteen ryhmään sarakkeen `C.id` mukaan:
...
Näille ryhmille lasketaan sitten tuotteiden määrä `COUNT(Pr.id)` sekä ostosten yhteishinta `SUM(T.price)`.

Huomaa, että kyselyssä ryhmittely tapahtuu sarakkeen `C.id` mukaan, mutta kyselyssä haetaan sarake `C.name`. Tämä on sinänsä järkevää, koska sarake `C.id` määrää sarakkeen `C.name`. Tämä kysely toimii SQLitessä, mutta muut tietokannat voivat vaatia, että sellaisenaan haettavan sarakkeen tulee aina esiintyä myös ryhmittelyssä. Tällöin ryhmittelyn tulisi olla `GROUP BY C.id, C.name`.
...
Rivin 4 sarakkeessa `teacher_id` on arvo `NULL`, joten jos suoritamme jommankumman äskeisen kyselyn, ongelmaksi tulee, että rivi 4 ei täsmää mihinkään taulun `Teachers` riviin. Tämän seurauksena tulostauluun ei tule riviä kurssista Tietokantojen perusteet:
...
FROM
  Courses LEFT JOIN Teachers
          ON Courses.teacher_id = Teachers.id
WHERE
  Courses.name <> 'Ohjelmoinnin perusteet';
```
...
SELECT
  C.name, COUNT(Pr.id), SUM(T.price)
FROM
  Customers C LEFT JOIN Purchases Pu ON C.id = Pu.customer_id
...
```sql
SELECT
  C.name, COUNT(Pr.id), IFNULL(SUM(T.price), 0)
FROM
  Customers C LEFT JOIN Purchases Pu ON C.id = Pu.customer_id
              LEFT JOIN Products Pr ON Pr.id = Pu.product_id
GROUP BY
...
```
name        COUNT(Pr.id)  IFNULL(SUM(T.price),0)
----------  ------------  ----------------------
Uolevi      2             9
Maija       3             19
Aapeli      0             0
