...
```sql
SELECT
  Courses.name, COUNT(Products.id), SUM(Products.price)
FROM
  Customers, Products, Purchases
WHERE
  Customers.id = Purchases.customer_id AND Products.id = Purchases.product_id
...
```
name        COUNT(id)  IFNULL(SUM(price),0)
----------  ------------  ----------------------
Uolevi      2             9
Maija       3             19
Aapeli      0             0
...
Funktio `ISNULL(x, y)` palauttaa arvon `x`, jos `x` ei ole `NULL`, ja muuten arvon `y`. Tämän avulla voidaan käsitellä tilanne, jossa SQL-kyselyssä oleva arvo saattaa olla `NULL`. Esimerkiksi `ISNULL(x, 0)` muuttaa arvon `x` nollaksi, jos se on `NULL`.

Funktio `ISNULL` ei kuulu SQL-standardiin, eikä se toimi kaikissa tietokannoissa. SQL-standardin mukainen funktio on `COALESCE`, jota voi käyttää samalla tavalla. Esimerkiksi `COALESCE(x, 0)` tarkoittaa samaa kuin `ISNULL(x, 0)`.
