...
```python
a = Movie("Lumikki", 1937)
b = Movie("Fantasia", 1940)
c = Movie("Pinocchio", 1940)
```
...
Muutoksena on, että taulussa `Courses` ei ole enää viittausta tauluun `Teachers`, mutta sen sijaan tietokannassa on uusi taulu `KurssinTeachers`, joka viittaa kumpaankin tauluun. Jokainen rivi tässä taulussa kuvaa yhden suhteen muotoa "kurssilla _id_ opettaa opettaja _id_".
...
Kun tietokantaan halutaan tallentaa listoja, luodaan uusi taulu, jossa jokainen rivi on jonkin listan yksittäinen alkio, kuten äskeinen taulu `KurssinTeachers`. Mutta miksi emme voisi vain tallentaa listaa yhteen sarakkeeseen? Seuraava esimerkki selventää asiaa.
...
```sql
SELECT student_id, SUBSTR(points, 1, 1) +
                   SUBSTR(points, 3, 1) +
                   SUBSTR(points, 5, 1) +
                   SUBSTR(points, 7, 1) FROM Results;
```
...
```
id          student_id   points1     points2     points3     points4
----------  -----------  ----------  ----------  ----------  ----------
1           1            6           5           1           4
2           2            3           6           6           6
3           3            6           4           0           6
...
```sql
SELECT credits FROM Students WHERE name = 'Maija';
```

Kuitenkin tietokannassa on toisteista tietoa: taulun `Students` sarakkeen `credits` sisältö voidaan laskea taulun `Completions` avulla. Ongelmana on, että jos lisäämme tai poistamme suorituksen, niin joudumme tekemään muutoksen kahteen eri tauluun. Jos muutos unohtuu tehdä tai epäonnistuu, tietokantaan tulee ristiriitaista tietoa.

Pääsemme eroon toisteisesta tiedosta poistamalla sarakkeen `credits` taulusta `Students`:
...
```sql
SELECT SUM(Completions.credits)
